...
- Open items: the Denma `orig_series_id` defect (accepted, not blocking); read the CI build's `volumes with a title` info line and its transition table before publishing; the local `build/opentome.db` needs a full rebuild to reflect the tier-0 title pick. Mangarr consumer follow-ups (separate repo, after publish): `SubtitleOf` should take the artifact title as its first candidate before the Google Books record; `MapGcdStatus` should map `stalled` to a visible Stalled state instead of falling back to AniList (which reads a still-running Japanese series as Continuing — safe but loses the signal).
